...
# --------------------------------------
# File: 03_scripts/02_random_forests/03_rf_global_outside_class_predictors.R
# Purpose:
# This script runs the implemented global outside-class random forest model used
# in the published Thailand PISA 2022 analysis. It estimates variable importance
# using all valid non-input, non-COVID predictors labeled "Out of School" or
...
# summaries, selected partial dependence plots, and a comparison table across
...
# 3. Select valid outside-class and in-and-out predictors.
...
# 7. Generate PDPs for the top global outside-class predictors.
# 8. Build a comparison table across the three global RF model variants.
#
...
compare_top20_paths <- list(
  all = file.path("PATH_TO_LOCAL_OUTPUT", "random_forests", "global", "all_predictors", "global_all_predictors_rf_top20_predictors_by_importance.csv"),
  in_class = file.path("PATH_TO_LOCAL_OUTPUT", "random_forests", "global", "in_class", "global_in_class_rf_top20_predictors_by_importance.csv"),
  outside_class = file.path("PATH_TO_LOCAL_OUTPUT", "random_forests", "global", "outside_class", "global_outside_class_rf_top20_predictors_by_importance.csv")
)
...
# Save to CSV
comparison_file <- file.path("PATH_TO_LOCAL_OUTPUT", "random_forests", "global", "global_rf_top20_comparison_table.csv")
