...
# --------------------------------------
# File: 03_scripts/02_random_forests/01_rf_global_all_predictors.R
# Purpose:
# This script runs the implemented global random forest model used in the
# published Thailand PISA 2022 analysis. It estimates variable importance using
# all valid non-input, non-COVID predictors across mathematics, reading, and
# science plausible values, then produces aggregated importance summaries and
# selected partial dependence plots.
#
# Main tasks:
# 1. Load the analysis-ready dataset and mapping file.
# 2. Exclude selected flag-based cases and non-eligible predictors.
# 3. Run random forest models across PV1–10 for MATH, READ, and SCIE.
# 4. Save raw and aggregated importance outputs and top-20 plots.
# 5. Update the model tracker.
# 6. Generate PDPs for the top global predictors.
#
...
for (outcome in outcomes) {
  ...
  # Public repository note:
  # The tracker file is part of the broader internal audit trail used in the project.
  # Tracker
...
importance_files <- list.files(
  ...
  pattern = "^global_all_predictors_.*_rf_variable_importance\\.csv$",
  full.names = TRUE
)

if (length(importance_files) != 3) {
  stop(
    "❌ Expected 3 variable importance files (MATH, READ, SCIE), but found: ",
    length(importance_files)
  )
}
...
if (nrow(all_top20) == 0) {
  stop("❌ all_top20 is empty after reading importance files.")
}
...
# Create a unified weighted bootstrap training dataset: 5000 rows for each of the 30 PVs
combined_df <- map_dfr(
  c("MATH", "READ", "SCIE"),
  function(subject) {
